...
export class UndoHistory {
    ...
    entry.store.makeChanges(() => {
      ...
      for (const id in entry.changes) {
        ...
        if (change.op === "create") {
          ...
          if (!obj) {
            const recreated = entry.store.objectFromFields(change.properties);
            entry.store.addObject(recreated);
  ...
  private onChange(store: AnyObjectStore, obj: AnyObject, origin: ChangeOrigin, change: ObjectChange) {
    ...
    if (this.pendingChanges?.store !== store) {
      // Don't mix changes from different stores in the same history entry.
      this.commit();
    }

    if (!this.pendingChanges) {
      this.pendingChanges = { store, changes: {} };
    }
